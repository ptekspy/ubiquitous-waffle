  ...
  return (
    <section className={`${cardClass} mb-[18px] grid gap-[22px] p-[22px] lg:grid-cols-[minmax(0,1fr)_minmax(300px,390px)]`}>
      <div>
        ...
        <p className={`${mutedClass} mb-[18px] max-w-3xl leading-relaxed`}>PaidPolitely opens the account page in a quiet tab, captures public post rows, removes noisy promo cards, and only brings Reddit forward if it needs your attention.</p>
      ...
      <ol className="grid content-start gap-2.5 p-0">
        ...
        <JourneyStep number={3} title="Capture" body="Open a quiet tab, scroll, and import metadata." state={captureStepState(extensionState, loading, hasData, canScan)} />
      ...
      <form className="rounded-3xl border border-white/12 bg-black/15 p-4 lg:col-span-2" onSubmit={submit}>
        ...
        <div className="mt-2.5 flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
          <small className="text-[#c9adbd]">{extensionReady ? "Extension ready. The account page will open quietly unless Reddit needs attention." : "Install or reload the extension, then recheck."}</small>
