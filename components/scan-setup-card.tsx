  ...
  return (
    <section className={`${cardClass} mb-[18px] grid gap-[22px] p-[22px] lg:grid-cols-[minmax(0,1fr)_minmax(300px,390px)]`}>
      <div>
        ...
        <h2 className="my-2.5 text-[clamp(1.8rem,4vw,3rem)] leading-none font-black tracking-[-0.05em]">Scan an account with the browser extension.</h2>
        <p className={`${mutedClass} mb-[18px] max-w-3xl leading-relaxed`}>PaidPolitely first pages through Reddit JSON without opening a tab. If Reddit blocks that, it falls back to a quiet tab and only brings Reddit forward when it needs your attention.</p>
      ...
      <ol className="grid content-start gap-2.5 p-0">
        ...
        <JourneyStep number={2} title="Username" body="Paste a username, account URL, or u/name." state={usernameStepState(hasValidUsername, extensionReady)} />
        <JourneyStep number={3} title="Capture" body="Page no-tab JSON, then quiet-tab fallback." state={captureStepState(extensionState, loading, hasData, canScan)} />
      ...
      <form className="rounded-3xl border border-white/12 bg-black/15 p-4 lg:col-span-2" onSubmit={submit}>
        <label className="mb-2 block text-sm font-extrabold text-[#c9adbd]" htmlFor="username">Account</label>
        <div className="grid gap-2.5 sm:grid-cols-[minmax(0,1fr)_auto]">
          ...
          <button className={primaryButtonClass} disabled={!canScan} type="submit">{extensionState === "scanning" || loading ? "Scanning..." : normalisedUsername ? `Scan u/${normalisedUsername}` : "Scan account"}</button>
        </div>
        <div className="mt-2.5 flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
          <small className="text-[#c9adbd]">{extensionReady ? "Extension ready. Most scans try paginated no-tab first; Reddit opens only if fallback is needed." : "Install or reload the extension, then recheck."}</small>
